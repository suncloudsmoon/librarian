...
sys.path.append("./src/bookshelf_manager/")
from utils import Category, ClassificationSystem
from catalog_manager import Book
import fitz
from io import BytesIO
from librarian import Librarian
from bookshelf_manager.catalog_manager import Metadata
...
def convert_old_librarian_v11(librarian_path):
    librarian = Librarian(librarian_path, default_config=None)
    ...
    catalog_dir = librarian_path / "catalog"
    for book in catalog:
        metadata = Metadata(
            id=str(uuid7()),
            type="book",
        ...
        path = catalog_dir / f"{metadata["id"]}".json
        contents = json.dumps(asdict(metadata))
...
def convert_old_librarian_v10(librarian_path):
    ...
    for metadata in catalog:
        ...
        if binary_data:
            librarian.catalog_manager.cover_image_manager.add(
                id, cover_image=binary_data
            )
